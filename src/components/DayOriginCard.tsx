...
export function DayOriginCard({ originPlace, originTime, onEditPlace, onUpdateTime, isPreview }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-emerald-200 bg-emerald-50 shadow-sm px-3 py-3 flex items-center gap-2.5">
      {/* Departure time */}
      {isPreview ? (
        <span className="font-mono text-xs w-10 flex-none text-gray-500 tabular-nums">{time}</span>
      ) : (
      ...
      {/* Place — clicking opens SearchOverlay */}
      {isPreview ? (
        <div className="flex-1 min-w-0">
          <p className={`text-sm truncate ${originPlace ? 'text-emerald-800' : 'text-gray-300'}`}>
            {originPlace?.name ?? '未设置出发地'}
          </p>
          {originPlace?.address && (
